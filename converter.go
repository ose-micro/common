...
func BuildGRPCToRequest(query *Request) (*commonv1.Request, error) {
	if query == nil {
...
func BuildRequestToGRPC(request *commonv1.Request) (*Request, error) {
	if request == nil {
		return nil, fmt.Errorf("request is nil")
	}

	facets := make([]Query, len(request.Facets))

	for i, facet := range request.Facets {
